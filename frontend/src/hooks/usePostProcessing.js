...
export function useBaseComposer(isVisible) {
...
export function useBloomComposer(isVisible) {
  ...
  useEffect(() => {
    ...
    return () => {
      try {
        ...
        if (originalResize) {
          composer.setSize = originalResize;
        }
      } catch (error) {
...
export function useMotionBlurComposer(isVisible) {
  const composer = useBaseComposer(isVisible);
