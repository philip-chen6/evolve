...
const Timeline = () => {
  ...
  useEffect(() => {
    if (!loading && timelineData.length > 0) {
      // Set up the animation timer for scroll unlocking
      const timer = setTimeout(() => {
        setIsAnimationComplete(true);
      }, 2500); // 2s animation + 0.5s delay

      // Show the first item immediately without waiting for the animation to finish
      if (displayedCount === 0) {
        setDisplayedCount(1);
      }
  ...
  }, [loading, timelineData, displayedCount]);
  ...
  useEffect(() => {
    ...
    if (container && isAnimationComplete) {
  ...
  }, [displayedCount, timelineData.length, isAnimationComplete]);
